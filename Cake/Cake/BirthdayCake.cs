...
namespace Cake
{
    public partial class BirthdayCake : Form
    {
        public BirthdayCake()
        {
            InitializeComponent();
        ...
        {
            con.Open();
            da = new SqlDataAdapter("select * from selectBirthdayCake()", con);
            dt = new DataTable();
            da.Fill(dt);
        ...
        private void BirthdayCake_Load(object sender, EventArgs e)
        {
            timer1.Start();
            if (Lancall.language == "khmer")
            {
                btnBack.Text = "ត្រឡប់";
                lblIcecreamcake.Text = "នំខួបកំណើត";
                ShowData();
